...
def reset(reset_current_class_polygon=False):
    global current_image, first_frame, window, current_polygon, points_bunch, current_class_index, class_namDaniele De Gregorioes
    current_polygon = None
    current_image = first_frame.copy()
    points_bunch = ThreePointsBoxBunch()
...
while(current_index < len(images)):
    ...
    # f.write("scan_01/images/{}.jpg ,".format(current_index))
...
while (count < len(images)):
    ...
    f.write(p.parts[-2] + '/' + p.parts[-1] + '/rgb_' + namestring + ".jpg ")

    count2= 0
    
    for k in polygons_flow_map.keys():
        
        elem = polygons_flow_map[k] # e.g. [[323,525,654,123],[423,12,434,343]]
        arr = elem[count].ravel()
        #print(arr)

        #for i range()
        #stringa = np.array2string(np.array(polygons_flow_map['bianco_normale'][count]), precision=2, separator=', ')
        #f.write("0," + str(stringa[1:-1]) + "\n")
        #f.write("0," + str(np.array(polygons_flow_map['bianco_normale'][count])) + "\n")

        f.write("{},{},{},{},{},{},{},{},{} ".format(count2, int(round(arr[0])), int(round(arr[1])), int(round(arr[2])), int(round(arr[3])), 
        int(round(arr[4])), int(round(arr[5])), int(round(arr[6])), int(round(arr[7])) ))
        count2 += 1
